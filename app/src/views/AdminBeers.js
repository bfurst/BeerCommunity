...
import { addFavorite, createBeer, deleteBeer, getAdminBeers, getBeerCategories, getBeers, getBeerShades, getBrewery, getReportCategories, getRestrictionCategories, removeFavorite, updateBeer } from '../services/Api';
...
export default function AdminBeers() {
    ...
    const [reportCategories, setReportCategories] = React.useState(null);
    const [restrictionCategories, setRestrictionCategories] = React.useState(null);
    const [page, setPage] = React.useState(1);
    const [showComments, setShowComments] = React.useState(false);
    const [selectedBeer, setSelectedBeer] = React.useState(null);
    const [showAddBeerModel, setShowAddBeerModel] = React.useState(false);
    const [showConfirmModal, setShowConfirmModal] = React.useState(false);
    const [showUpdateBeerModel, setShowUpdateBeerModel] = React.useState(false);
    ...
    const getBasicData = async () => {
        try {
            ...
            setRestrictionCategories(restrictionCategories);
    ...
    const getBeerData = async (pageNumber) => {
        try {
            const beers = await getAdminBeers(id, pageNumber, searchTerm, sortOrder, category, shade);
    ...
    const addBeer = async (data) => {
        try {
            ...
            formData.append("beerData", JSON.stringify(beer));
    ...
    const passBeerData = (beer) => {
        const updatedBeer = Object.assign({}, beer);
        updatedBeer.image = `https://localhost/uploads/beers/${updatedBeer.image}`
        setSelectedBeer(updatedBeer);
        setShowUpdateBeerModel(true);
    }

    const updateBeerData = async (data) => {
        const beer = {
            "id": selectedBeer.id,
            "name": data["name"],
            "alcoholPercentage": data["alcoholPercentage"],
    ...
    if (displayData === null || brewery === null || displayCategories === null || displayShades === null || reportCategories === null || restrictionCategories === null)
        return <Loading />;
        ...
        <Container className="d-flex flex-column justify-content-center">
            ...
            <div className="d-flex flex-wrap flex-column flex-md-row justify-content-between align-items-start gap-3 my-5">
                ...
                <div className="d-flex flex-wrap flex-md-nowrap gap-3 w-md-auto">
                    ...
                    <Col xs={12} md="auto">
                        <Button className="btn-success w-100" onClick={() => setShowAddBeerModel(true)}>
                            Add Beer
                        </Button>
                    </Col>
                </div>
                                    ...
                                    <Card className={beer.isFavorite ? "mb-4 shadow-sm border-2 border-danger" : "mb-4 shadow-sm"} style={{ marginRight: 5, width: 380 }}>
                                        ...
                                        <Col className="d-flex flex-column justify-content-between h-100">
                                            <Card.Body className="text-start mt-0 mb-auto">
                                                <div className="d-flex justify-content-between align-items-start">
                                                    ...
                                                    <Dropdown>
                                                        ...
                                                        <Dropdown.Menu>
                                                            ...
                                                            <Dropdown.Item eventKey="3" onClick={() => passBeerData(beer)}>Edit</Dropdown.Item>
                                                            <Dropdown.Divider />
                                                            <Dropdown.Item eventKey="4" onClick={() => { setSelectedBeer(beer); setShowConfirmModal(true) }}>Delete</Dropdown.Item>
                                                        </Dropdown.Menu>
                                                    </Dropdown>
                                                </div>
                                                <p>
                                                ...
                                                {
                                                    beer.numberOfReports > 0 &&
                                                    <div className="d-flex align-items-center mb-3">
                                                        <Icon.Flag size={24} color="red" title="Reported reviews" />
                                                        <span>&nbsp;<b>{beer.numberOfReports}</b>&nbsp;Reported reviews</span>
                                                    </div>
                                                }
                                                <div className="d-flex align-items-center mb-3">
            ...
            <ConfirmModal
                ...
                message="Are you sure you want to delete beer?"
                reviewId={selectedBeer !== null && selectedBeer.id}
                onConfirm={removeBeer}
                onClose={() => setShowConfirmModal(false)}
            />

            <BeerModal
                show={showAddBeerModel}
                data={null}
                categories={categories}
                shades={shades}
                onConfirm={addBeer}
                onClose={() => setShowAddBeerModel(false)}
            />

            <BeerModal
                show={showUpdateBeerModel}
                data={selectedBeer}
                categories={categories}
                shades={shades}
                onConfirm={updateBeerData}
                onClose={() => setShowUpdateBeerModel(false)}
            />
